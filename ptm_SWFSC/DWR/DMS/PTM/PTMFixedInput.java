...
/**
 * This class is contained in PTMEnv class. The main purpose of this class is reading fixed input.
 * <p>
 * The Waterbody and Node relationship is defined by a network structure. Each Waterbody has a unique id and maybe queried as to which nodes it is
 * connected to. Each Node also has a unique id and can be queried as to which waterbodies it is connected to.
 * <p>
 * Calls are made to fortran functions for reading fixed input files. These fortran functions read the data into common blocks. The information is
 * then transferred with a C++-Fortran function calls
 * <p>
 * This class is also called upon by PTMEnv class to create Waterbody, Node and XSection objects and insert them into arrays.
 * <p>
 * 
 * @author Nicky Sandhu
...
public class PTMFixedInput {
	/**
	 * Constructor: Opens the fixed input information files and fills the fortran common block with the information
	 */
	...
	/**
	 * Creates the Waterbody array. The assumption here is that channels are stored first, followed by reservoirs, stage boundaries, boundaries and
	 * lastly conveyors
	 */
	...
	private final Channel createChannel(int nId) {
		...
		if (len > 0) {
			...
			wb = new SmartChannel(lId, nId, xSectionIds, len, nodeArray, xSectDist);
			if (DEBUG) {
	...
	private final Reservoir createReservoir(int nId) {
		...
		if (area > 1e-10f) {
			...
			int[] nodeArray = _fixedData.getNodeArrayForWaterbody(nId);
			wb = new Reservoir(nId, lId, name, area, botelv, nodeArray);
		}
		return wb;
	}
	 ...
	 * 
	 * Note however that Reservoir Node info, ag. drain and pump Node info is not directly available from the common block This Node info is present
	 * in the common block for the Reservoir and for ag. drains is the Node number itself...
	 * 
	 * It is the responsibility of the PTMEnv class to calculate and update the Node object with this information.
	 */
	public final Node[] createNodeFixedInfo() {
		...
		for (int i = 1; i <= nodeArray.length; i++) {
			...
			if (DEBUG)
				System.out.println("Created node: " + nodeArray[i]);
		}
		return nodeArray;
	}

	/**
	 * Creates the cross section array with fixed information from common block
	 */
	public final XSection[] createXSectionFixedInfo() {
		...
		for (i = 1; i <= PTMFixedData.getMaximumNumberOfXSections(); i++) {
			...
			if (width[0] > 0) {
				...
				xSArray[i] = new XSection(i, numElvs, dist, width, area, elevation, minElv, false);
			}
		}
		return xSArray;
	}
	...
	/**
	 * updates the class ParticleFixedInfo with fixed information from common block
	 */
	public final void getPTMFixedInfo(ParticleFixedInfo info) {
		...
		info.setVariables(random_seed, trans_constant, vert_constant, trans_a_coef, trans_b_coef, trans_c_coef, animated_particles);

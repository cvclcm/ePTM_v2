...
/**
 * Records particles critical events, e.g. insertion, change of waterbody, change of node, time interval change, death ??only the first 2 & the last 1
 * are implemented
 * 
 */
class ParticleObserver {
	 ...
	 * Constructor
	 */
	public ParticleObserver(String traceFileName, int outputType, int startTime, int endTime, int PTMTimeStep, int nParticles) {
		traceOn = true;
		try {
			if (traceOn)
				output = new PTMTraceOutput(traceFileName, outputType, startTime, endTime, PTMTimeStep, nParticles);
	...
	/**
	 * destructor public ~ParticleObserver(){ delete output; }
	 */
	...
	/**
	 * observes the fact that the Particle has entered a Waterbody after encountering a node
	 */
	...
	/**
	 * observes the fact that a certain time interval has gone by since the last observed time...
	 */
	...
	/**
	 * shows the fact that the Particle has entered a Waterbody after encountering a node
	 */
	public void showWaterbodyChange(Particle observed) {
		if (traceOn) {
			...
			System.out.println(
					"Particle " + "  " + observed.getId() + " in waterbody " + "  " + wb.getEnvIndex() + " which is a " + "  " + wb.getPTMType());
			System.out.flush();
		}
	}
	...
	public void showNodeChange(Particle observed) {
		...
		System.out.println("Particle " + "  " + observed.getId() + " at Node " + "  " + observed.getRecentNode().getEnvIndex());
	...
	/**
	 * shows the fact that a certain time interval has gone by since the last observed time...
	 */
